    ...
    class Command
      def run(argv)
        ...
        OptionParser.new do |opts|
          ...
          opts.on('-o', '--output [file]', 'output filename, path, or url (if control_repo type is specified)') do |output|
          ...
          opts.on('-B', '--destination_branch [branch_name]', 'destination branch to use (control-repo output only)') do |branch_name|
            options[:destination_branch] = branch_name
          end
