    ...
    class Command
      def run(argv)
        ...
        OptionParser.new do |opts|
          ...
          opts.on('-e', '--edition [edition_name]', 'SIMP Edition', 'valid editions:', '  - community', '  - enterprise') { |opt| options[:edition] = opt }
          ...
          opts.on('-u', '--url [url]', 'URL of git hosting server to use (control-repo output only)') { |opt| options[:url] = opt }
          ...
          opts.on('-d', '--debug [lvl]', 'debug logging level: critical, error, warning, info, debug1, debug2') { |opt| Simp::Metadata::Debug.debug_level(opt) }
